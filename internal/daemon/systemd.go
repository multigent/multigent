...
func (m *systemdManager) buildUnit(cfg Config) string {
	...
	fmt.Fprintf(&sb, "ExecStart=%s start", cfg.BinaryPath)
	if cfg.ConfigPath != "" {
		fmt.Fprintf(&sb, " --config %s", cfg.ConfigPath)
	}
	fmt.Fprintf(&sb, " --dir %s --addr %s\n", cfg.WorkDir, cfg.Addr)
	fmt.Fprintf(&sb, "WorkingDirectory=%s\n", cfg.WorkDir)
	sb.WriteString("Restart=on-failure\n")
	sb.WriteString("RestartSec=10\n")
	fmt.Fprintf(&sb, "Environment=MULTIGENT_LOG_FILE=%s\n", cfg.LogFile)
	fmt.Fprintf(&sb, "Environment=MULTIGENT_LOG_MAX_SIZE=%d\n", cfg.LogMaxSize)
	if cfg.EnvPATH != "" {
...
func checkSystemdRunning(system bool) error {
	...
	if system {
		if wsl {
			return fmt.Errorf("systemd is not active in this WSL2 instance.\n" +
				...
				"    [boot]\n" +
				"    systemd=true\n" +
				"  Or use: nohup multigent start > multigent.log 2>&1 &")
		}
		if state == "offline" || strings.Contains(state, "not been booted") {
			return fmt.Errorf("systemd is not active (state: %s).\n"+
				...
				"    nohup multigent start > multigent.log 2>&1 &", state)
		}
		return fmt.Errorf("systemd check failed (state: %s).\n"+
			"  Use nohup as alternative: nohup multigent start > multigent.log 2>&1 &", state)
	}
	...
	return fmt.Errorf("systemd user session not available.\n"+
		...
		"  Try one of:\n"+
		"    1. Run as root: sudo multigent daemon install (uses system-level systemd)\n"+
		"    2. loginctl enable-linger %s && export XDG_RUNTIME_DIR=/run/user/$(id -u)\n"+
		"    3. Use nohup/tmux instead: nohup multigent start > multigent.log 2>&1 &", user)
}
