...
func (r *Runner) workflowPromptContext(project, agentName, taskID string) string {
	...
	b.WriteString("- Treat the current step as the milestone contract for this task: it defines the current goal, expected inputs, completion criteria, and structured handoff shape.\n")
	b.WriteString("- You do not have to finish this workflow step in this single wakeup. Long-running work may span multiple wakeups, conversations, code iterations, external coordination, and human clarification.\n")
	b.WriteString("- If you can safely complete the step now, do it and advance the workflow with structured outputs. If not, keep the task state honest: communicate blockers, ask the right people, record what you are waiting for, or leave clear progress notes before ending the run.\n")
	...
	b.WriteString("- Do not treat a natural-language clarifying question as workflow completion. Ask or discuss when needed, then either continue in a later wakeup or explicitly mark the workflow step done/failed only when the milestone outcome is actually clear.\n")
	...
	b.WriteString("- If this step needs human review, external clarification, or another collaborator before safe completion, contact them through the available channel or use `mga task confirm-request`; do not silently spin or pretend the milestone is complete.\n")
