...
func videoProductionStudio(locale string) entity.PlaybookTemplate {
	...
	return entity.PlaybookTemplate{
		...
		Roles: []entity.PlaybookRoleTemplate{
			roleWithPrompt("video-producer", "creative", "producer", text(locale, "Producer", "制片 Agent"), text(locale, "Owns the brief, audience, budget, approval gates, and final delivery shape.", "负责需求简报、受众、预算、审核门禁和最终交付形式。"), openMontageRolePrompt(locale, "producer", "executive-producer", "creative-intake", "checkpoint-protocol"), []string{"video-creative-intake", "video-production-checkpoint"}),
			roleWithPrompt("creative-director", "creative", "creative-director", text(locale, "Creative Director", "创意导演 Agent"), text(locale, "Turns the brief and references into a concrete concept, style direction, and production path.", "把需求和参考转成具体创意方案、风格方向和制作路径。"), openMontageRolePrompt(locale, "creative-director", "proposal-director", "video-reference-analyst", "taste-direction"), []string{"video-reference-analysis", "video-proposal"}),
			roleWithPrompt("script-writer", "creative", "script-writer", text(locale, "Script Writer", "脚本 Agent"), text(locale, "Writes the script, narration, pacing, and message hierarchy for the approved concept.", "为已审核创意编写脚本、旁白、节奏和信息层级。"), openMontageRolePrompt(locale, "script-writer", "script-director", "voice-performance-director"), []string{"video-script-writing"}),
			roleWithPrompt("storyboard-planner", "creative", "storyboard-planner", text(locale, "Storyboard Planner", "分镜 Agent"), text(locale, "Converts script into scenes, asset requirements, visual beats, and reviewable storyboard docs.", "把脚本转成场景、素材需求、视觉节拍和可审核分镜文档。"), openMontageRolePrompt(locale, "storyboard-planner", "scene-director"), []string{"video-storyboard-planning"}),
			roleWithPrompt("asset-director", "creative", "asset-director", text(locale, "Asset Director", "素材导演 Agent"), text(locale, "Plans and produces image, video, audio, music, and subtitle assets with provenance and cost notes.", "规划并产出图片、视频、音频、音乐和字幕素材，记录来源和成本。"), openMontageRolePrompt(locale, "asset-director", "asset-director", "animation-runtime-selector"), []string{"video-asset-manifest", "video-tool-selection"}),
			roleWithPrompt("editor-composer", "creative", "editor-composer", text(locale, "Editor / Composer", "剪辑合成 Agent"), text(locale, "Builds edit decisions, composition plan, render package, and technical validation evidence.", "产出剪辑决策、合成方案、渲染包和技术校验证据。"), openMontageRolePrompt(locale, "editor-composer", "edit-director", "compose-director"), []string{"video-edit-plan", "video-render-qa"}),
			roleWithPrompt("production-reviewer", "creative", "reviewer", text(locale, "Production Reviewer", "制作审核 Agent"), text(locale, "Reviews brand fit, content accuracy, visual quality, audio quality, delivery promise, and publish readiness.", "审核品牌一致性、内容准确性、画面质量、音频质量、交付承诺和发布就绪度。"), openMontageRolePrompt(locale, "production-reviewer", "reviewer", "publish-director"), []string{"video-production-review"}),
		},
		Skills: []entity.PlaybookSkillTemplate{
			openMontageSkill("video-creative-intake", text(locale, "Creative Intake", "创意需求收集"), text(locale, "Turn a vague video request into target audience, message, format, constraints, and approval policy.", "把模糊视频需求整理成目标受众、核心信息、格式、约束和审核策略。"), "creative-intake"),
			openMontageSkill("video-reference-analysis", text(locale, "Reference Analysis", "参考分析"), text(locale, "Analyze reference videos or examples without copying them; extract structure, pacing, style, and production implications.", "分析参考视频或案例但不照搬，提取结构、节奏、风格和制作含义。"), "video-reference-analyst"),
			openMontageSkill("video-proposal", text(locale, "Video Proposal", "视频提案"), text(locale, "Create differentiated concepts with tool path, budget range, risk, and recommendation.", "产出差异化创意方案，包含工具路径、预算区间、风险和推荐选择。"), "proposal-director"),
			openMontageSkill("video-script-writing", text(locale, "Script Writing", "脚本写作"), text(locale, "Write a timed script with hook, beats, narration, screen text, and constraints for downstream scenes.", "编写带时长的脚本，包含开场钩子、节拍、旁白、屏幕文字和下游分镜约束。"), "script-director"),
			openMontageSkill("video-storyboard-planning", text(locale, "Storyboard Planning", "分镜规划"), text(locale, "Turn script into reviewable scenes, shot intent, asset needs, and acceptance criteria.", "把脚本转成可审核场景、镜头意图、素材需求和验收标准。"), "scene-director"),
			openMontageSkill("video-asset-manifest", text(locale, "Asset Manifest", "素材清单"), text(locale, "Produce an asset manifest with source, license, generation prompt, cost, quality status, and docID references.", "产出素材清单，包含来源、授权、生成提示、成本、质量状态和 docID 引用。"), "asset-director"),
			openMontageSkill("video-tool-selection", text(locale, "Tool Selection", "工具选择"), text(locale, "Choose providers and tools based on quality, cost, speed, credentials, and approval policy.", "按质量、成本、速度、凭证和审核策略选择 provider 与工具。"), "animation-runtime-selector"),
			openMontageSkill("video-edit-plan", text(locale, "Edit Plan", "剪辑方案"), text(locale, "Create timeline, captions, music, transitions, and render instructions from approved assets.", "基于已审核素材创建时间线、字幕、音乐、转场和渲染说明。"), "edit-director"),
			openMontageSkill("video-render-qa", text(locale, "Render QA", "成片质检"), text(locale, "Check output format, duration, audio, subtitles, readability, visual defects, and delivery promise.", "检查输出格式、时长、音频、字幕、可读性、画面缺陷和交付承诺。"), "compose-director"),
			openMontageSkill("video-production-checkpoint", text(locale, "Production Checkpoint", "制作检查点"), text(locale, "Use checkpoint docs to support resume, review, cost tracking, and audit trail.", "用检查点文档支持恢复、审核、成本跟踪和审计记录。"), "checkpoint-protocol"),
			openMontageSkill("video-production-review", text(locale, "Production Review", "制作审核"), text(locale, "Review creative quality and production readiness with explicit approve/request_changes decisions.", "用明确的 approve/request_changes 决策审核创意质量和制作就绪度。"), "reviewer"),
		},
		Workflows: []entity.PlaybookWorkflowTemplate{
...
func openMontageSkill(id, name, description, assetName string) entity.PlaybookSkillTemplate {
	return entity.PlaybookSkillTemplate{
		ID:          id,
		Name:        name,
		Description: description,
		Body:        openMontageAdapterPrefix(assetName) + openMontageSkillBody(assetName),
		Source:      "Adapted from https://github.com/calesthio/OpenMontage skills/" + assetName,
		LicenseNote: "AGPLv3. Source vendored under internal/playbook/openmontage_assets/LICENSE. Review licensing before commercial distribution.",
	}
}

func openMontageRolePrompt(locale, roleID string, assetNames ...string) string {
	header := text(locale,
		"# Role prompt source\n\nThis role is backed by selected OpenMontage operating procedures, adapted for Multigent's docID-based workflow, human review gates, and external tool connections.\n\n",
		"# Role Prompt 来源\n\n这个角色基于选定的 OpenMontage 操作规程，并适配 Multigent 的 docID 工作流、人工审核门禁和外部工具连接。\n\n",
	)
	parts := []string{header, text(locale,
		"## Multigent Role Adapter\n\n- Treat workflow input/output fields as the source of truth.\n- Store long artifacts in Multigent Docs and return docIDs.\n- Do not expose local filesystem paths to end users.\n- Stop at human review gates and carry review comments into the next pass.\n- If OpenMontage mentions local project paths or Python tools, translate the intent into Multigent docs, tool connections, and sandbox execution.\n\n",
		"## Multigent 角色适配\n\n- 以 workflow 输入/输出字段作为事实来源。\n- 大段产物写入 Multigent 知识库并返回 docID。\n- 不向最终用户暴露本地文件路径。\n- 遇到人工审核门禁要停下，并把审核意见带入下一轮。\n- 如果 OpenMontage 提到本地项目路径或 Python 工具，把意图转换成 Multigent docs、工具连接和 sandbox 执行。\n\n",
	)}
	for _, name := range assetNames {
		parts = append(parts, fmt.Sprintf("\n\n---\n\n## OpenMontage Procedure: %s\n\n%s", name, openMontageSkillBody(name)))
	}
	return strings.Join(parts, "")
}

func openMontageAdapterPrefix(assetName string) string {
	return fmt.Sprintf(`# Multigent Adapter

This skill is adapted from OpenMontage's "%s" operating procedure.

Use it inside Multigent with these changes:
- Long artifacts must be created as Multigent knowledge docs and returned by docID.
- Workflow input/output specs are binding.
- Human review gates must stop the workflow until approve/request_changes/stop is recorded.
- Local file paths in the upstream text should be translated to Multigent artifact references, object storage references, or sandbox-internal paths.
- Tool/provider references describe capability needs; actual execution must use Multigent-approved external tool connections and sandbox runtime.
