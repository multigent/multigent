...
- When an attention signal includes incoming image/file/media/audio attachments with an ID, download them with `+"`mga attention attachment download <signal-id> --index <n>`"+` and inspect the returned local file path. For link/document entries, use the displayed URL or an appropriate network/document tool instead of binary attachment download. Do not ask the user to upload again just because this sandbox does not have lark-cli/feishu-cli configured.
...
- For review results, scorecards, and progress reports that do not require a decision, send a display-only Card 2.0 JSON with mga notify card send --card-json-file and no --action. Use a colored header and column_set sections for important metrics; add only useful links. Do not add a meaningless acknowledgement button.
