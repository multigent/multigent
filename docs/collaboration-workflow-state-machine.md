...
- `allowed`：允许执行过程中拆出子 Task 分支。
- `disabled`：强制单 Task 执行，适合极小任务。
- `requires_approval`：拆分前需要负责人确认。

这样用户不用每次从零设计流程，也不会被迫让每个需求都走并行分支。

### Task Branch 与 Workflow Branch

这里不应该把问题理解成“从几条预设流程里选择一条”。

更准确的理解是：

```text
Task 可以在执行过程中拆出分支；
Workflow 负责记录这些分支如何产生、由谁执行、何时汇合。
```

也就是说，Task 的分支和 Workflow 的分支应该是同一件事的两个视角：

- 用户视角：一个父 Task 拆成多个子 Task。
- 流程视角：一个 Workflow Run 出现多个并行 Step Instance 或 Child Workflow Run。
...
父 Task
  -> 父 Workflow Run
      -> Branch Group
          -> 子 Task A -> 子 Workflow Run A（可选）
          -> 子 Task B -> 子 Workflow Run B（可选）
          -> 子 Task C -> 子 Workflow Run C（可选）
      -> Join Step
```
...
这两种情况应该由同一套模型表达，而不是引入两套机制。

### Branch Step

为了兼容“有些需求需要拆，有些不需要拆”，可以定义一个 branch step。

branch step 的职责不是把流程切到某条固定路径，而是输出一个分支计划：
...
父 Task：实现 Web Search 外部工具
  Branch Step 输出：
    - 子 Task A：实现 Brave Search
    - 子 Task B：实现 Exa Search
    - 子 Task C：补充测试用例
```
...
这才符合真实工作流：分支是任务执行中的一种可能形态，而不是所有需求都必须经过的固定流程。

### Branch 的创建方式

分支可以由三种方式产生：

1. Human 手动拆分：负责人在 Web 上把父 Task 拆成多个子 Task。
2. Agent 建议拆分：agent 输出 branch plan，请负责人确认。
3. 系统模板建议：某类流程在特定节点提示“可拆分”，但不自动强制拆分。

第一版建议先支持：

```text
Agent 建议拆分 -> Human 确认 -> 系统创建子 Task
```

这样不会让 agent 擅自制造大量任务，也符合企业流程里负责人控制范围的习惯。

### 并行与子 Task 的关系

并行 workflow 不应该是所有需求的默认路径。

它只在 branch step 确认需要拆分时出现。
...
steps:
  ...
  - id: branch_planning
    type: branch
    title: 分支规划
    output_schema: branch_plan_v1
    next:
      - when: split_required == false
        to: implementation
      - when: split_required == true
        to: implementation_parallel
