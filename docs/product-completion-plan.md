...
- OAuth authorization start/callback/token exchange now exists for OAuth-capable providers. The flow stores short-lived state server-side, exchanges authorization codes server-side, creates OAuth connections with encrypted tokens, and the Web connection dialog can start the OAuth flow.
- Remaining work: OAuth callback UX polish, token refresh, DingTalk enterprise app/OAuth provider, GitHub app installation flow, action catalog/schema support, and deeper per-provider executor hardening.
